...
#    imdb.set_proposal_method(args.method)
    ...
    print 'AverageRec: {}'.format(results['ar'])

    def recall_at(t):
        ind = np.where(results['thresholds'] > t - 1e-5)[0][0]
        assert np.isclose(results['thresholds'][ind], t)
        return results['recalls'][ind]
